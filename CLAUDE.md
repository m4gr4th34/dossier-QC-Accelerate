...
- Living figures (interactive SVG): authored as a `data-figure` JSON spec that
  ...
  the reference and walkthrough (incl. *Adopting the engine for a new figure type*).
  Two standing rules: **compose, don't reimplement** (render modules call the shared
  general primitives, never re-roll PRNG / scatter / log-zoom; domain primitives live
  in the consuming module), and **seal after editing** (run `render-figures` after any
